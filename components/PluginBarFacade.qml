...
// Project-local v4.0.2-compatible equivalent of the newer PluginBarApi.
// Only scalar presentation state and entry-scoped callbacks are exposed.
QtObject {
  id: api
  ...
  // v4.0.2 panels assign this property directly before closing. Keep that
  // contract writable and forward only changes, without exposing the host.
  property bool centerHoverRevealSuppressed: false
  on_CenterHoverRevealSuppressedChanged: {
    if (centerHoverRevealSuppressed !== _centerHoverRevealSuppressed)
      centerHoverRevealSuppressed = _centerHoverRevealSuppressed
  }
