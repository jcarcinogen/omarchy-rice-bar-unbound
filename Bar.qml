...
Item {
  ...
  // These values arrive after asynchronous custom-bar construction on 4.0.2.
  ...
  readonly property bool hostReady: barWidgetRegistry !== null && barConfig !== null && barWidgetRegistry.widgets !== undefined
  property var layoutConfig: fallbackBarConfig.layout
  readonly property var visualLayout: UnboundModel.rawLayout(barConfig && barConfig.layout)
  readonly property var visualSections: UnboundModel.mapSectionCapsules(visualLayout)
  ...
  function publicLayoutConfig() {
    var currentConfig = root.barConfig
  ...
  function widgetMetadata(moduleName) {
    var id = root.canonicalWidgetId(moduleName)
    if (!root.barWidgetRegistry) return null
    if (typeof root.barWidgetRegistry.metadataFor === "function")
      return root.barWidgetRegistry.metadataFor(id)
    var widgets = root.barWidgetRegistry.widgets || ({})
    return widgets[id] ? widgets[id].metadata || null : null
  }

  function widgetIsFirstParty(moduleName, registryMetadata) {
    var metadata = registryMetadata || widgetMetadata(moduleName)
    if (!metadata) return false
    if (metadata.firstParty !== undefined) return metadata.firstParty === true
    // v4.0.2 predates the explicit firstParty bit. Its registry-supplied
    // sourceDir is scanner-owned, so bundled plugin paths are a safe fallback.
    var sourceDir = String(metadata.sourceDir || "")
    var trustedDir = String(root.omarchyPath || "") + "/shell/plugins/"
    return trustedDir !== "/shell/plugins/" && sourceDir.indexOf(trustedDir) === 0
  }

  function updateScopedEntry(region, sourceIndex, moduleName, settings) {
    if (!root.shell || typeof root.shell.mutateShellConfig !== "function") return false
    var changed = false
    root.shell.mutateShellConfig(function(config) {
      var entries = root.rawLayoutSection(config, region)
      var index = Math.round(Number(sourceIndex))
      if (index < 0 || index >= entries.length || root.entryId(entries[index]) !== moduleName) return
      var next = { id: moduleName }
      var values = settings && typeof settings === "object" ? settings : ({})
      for (var key in values) if (key !== "id") next[key] = values[key]
      if (JSON.stringify(entries[index]) === JSON.stringify(next)) return
      entries[index] = next
      changed = true
    })
    return changed
  ...
  // Leave room around the hosted widgets for capsule paint on either axis.
  readonly property int barSize: vertical ? Style.bar.sizeVertical : 30
  readonly property int surfaceSize: vertical ? Math.max(60, barSize + 12) : 40
  ...
  function applySettingsDelta(delta) {
    for (var i = 0; i < delta.length; i++) {
      ...
      change.instanceKey = change.region + ":" + sourceIndexFor(change.region, change.entry, change.index)
      var settings = entrySettings(change.entry)
      for (var s = 0; s < moduleSlots.length; s++) {
        var slot = moduleSlots[s]
        if (!slot || slot.instanceKey !== change.instanceKey) continue
  ...
  function panelNavigationSlots(region, window) {
    ...
    for (var i = 0; i < entries.length; i++) {
      var instanceKey = region + ":" + sourceIndexFor(region, entries[i], i)
      for (var j = 0; j < moduleSlots.length; j++) {
        var slot = moduleSlots[j]
        if (!slot || slot.instanceKey !== instanceKey) continue
  ...
  function sourceIndexFor(region, entry, normalizedIndex) {
    var config = Util.isPlainObject(barConfig) ? barConfig : fallbackBarConfig
    var rawLayout = Util.isPlainObject(config.layout) ? config.layout : fallbackBarConfig.layout
    var sourceEntries = Array.isArray(rawLayout[region]) ? rawLayout[region] : []
    return BarModel.sourceIndexFor(sourceEntries, layoutEntries(region), normalizedIndex)
  }
  ...
  function dropBarModule(source, toRegion, beforeIndex) {
    if (!source || !source.region || source.sourceIndex < 0 || !toRegion) return false
    if (!root.shell || typeof root.shell.mutateShellConfig !== "function") return false

    var changed = false
    root.shell.mutateShellConfig(function(config) {
      rawLayoutSection(config, source.region)
      var destination = rawLayoutSection(config, toRegion)
      var targetIndex = beforeIndex < 0 ? destination.length : beforeIndex
      changed = BarModel.moveEntry(config, source.region, source.sourceIndex, toRegion, targetIndex)
    })
    return changed
  }
  ...
  function dropBarModuleAtTarget(sourceSlot, targetSlot, afterTarget) {
    ...
    var beforeIndex = targetSlot.sourceIndex + (afterTarget ? 1 : 0)
    return dropBarModule(sourceSlot, targetSlot.region, beforeIndex)
  }
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -root.surfaceSize : 0
      bottom: root.barHidden && root.position === "bottom" ? -root.surfaceSize : 0
      left: root.barHidden && root.position === "left" ? -root.surfaceSize : 0
      right: root.barHidden && root.position === "right" ? -root.surfaceSize : 0
    }
    ...
    implicitWidth: root.vertical ? root.surfaceSize : 0
    implicitHeight: root.vertical ? 0 : root.surfaceSize
    ...
    Loader {
      anchors.fill: parent
      sourceComponent: !root.hostReady ? diagnosticBar : (root.vertical ? verticalBar : horizontalBar)
    ...
    Component {
      ...
      Rectangle {
        ...
        Text {
          anchors.centerIn: parent
          text: "Rice Bar Unbound: waiting for Omarchy bar services"
          textFormat: Text.PlainText
    ...
    Component {
      ...
      Styles.NisfereInspired {
        anchors.fill: parent
        transparent: root.transparent
        leftComponent: unboundLeftComponent
        centerComponent: unboundCenterComponent
        rightComponent: unboundRightComponent
      }
    }

    Component {
      id: unboundLeftComponent
      SectionCapsules { groups: root.visualSections.left }
    }

    Component {
      id: unboundCenterComponent
      SectionCapsules {
        groups: root.visualSections.center
        HoverHandler { onHoveredChanged: root.setCenterSectionHovered(hovered) }
      }
    }

    Component {
      id: unboundRightComponent
      SectionCapsules { groups: root.visualSections.right }
    }
    ...
    Component {
      ...
      Item {
        ...
        id: sideBar
        Styles.VisualTokens { id: sideTokens }
        Rectangle {
          anchors.fill: parent
          color: root.transparent ? "transparent" : sideTokens.stripColor
        }
        readonly property real sideCapacity: Math.max(0, (height - sideCenter.height) / 2 - 16)

        VerticalSectionCapsules {
          id: sideCenter
          groups: root.visualSections.center
          anchors.centerIn: parent
          width: parent.width - 8
          height: Math.min(implicitHeight, Math.max(0, parent.height / 3))
          HoverHandler { onHoveredChanged: root.setCenterSectionHovered(hovered) }
        }
        VerticalSectionCapsules {
          groups: root.visualSections.left
          anchors.top: parent.top
          anchors.topMargin: 8
          anchors.horizontalCenter: parent.horizontalCenter
          width: parent.width - 8
          height: Math.min(implicitHeight, sideBar.sideCapacity)
        }
        VerticalSectionCapsules {
          groups: root.visualSections.right
    ...
    Component {
      ...
      Item {
        ...
        ModuleSlot {
          ...
          region: "center"
          sourceIndex: root.sourceIndexFor("center", centerRoot.anchorEntry, root.entryIndex(centerRoot.entries, root.centerAnchor))
          anchors.centerIn: parent
        }
        ...
        ModuleList {
          ...
          region: "center"
          sourceOffset: root.entryIndex(centerRoot.entries, root.centerAnchor) + 1
      ...
      Row {
        ...
        Repeater {
          ...
          ModuleSlot {
            ...
            sourceIndex: root.sourceIndexFor(moduleListRoot.region, modelData, moduleListRoot.sourceOffset + index)
          }
        }
      }
    }
  ...
  // Capsules never relocate configured widgets. Each lane can scroll when
  // space is tight, with an explicit paging button that cannot be clipped away.
  component VerticalSectionCapsules: Item {
    id: section
    property var groups: []
    implicitHeight: capsules.implicitHeight
    readonly property bool overflowing: implicitHeight > height
    Styles.VisualTokens { id: sideTokens }

    Flickable {
      id: scroller
      anchors.fill: parent
      anchors.bottomMargin: section.overflowing ? 24 : 0
      contentWidth: width
      contentHeight: capsules.implicitHeight
      clip: true
      ...
      Column {
        ...
        Repeater {
          ...
          Rectangle {
            ...
            height: modules.implicitHeight > 0 ? modules.implicitHeight + 8 : 0
            color: sideTokens.capsuleTop
            radius: Math.min(sideTokens.radius, width / 2)
            clip: true
            GroupedModuleList {
              id: modules
              anchors.centerIn: parent
              moduleRows: parent.modelData.rows
            }
          }
        }
      }
    ...
    Text {
      ...
      MouseArea {
        ...
        onClicked: scroller.contentY = scroller.atYEnd ? 0
          : Math.min(scroller.contentHeight - scroller.height, scroller.contentY + scroller.height)
      }
    }
  }

  component SectionCapsules: Item {
    id: section
    property var groups: []
    implicitWidth: capsuleRow.implicitWidth
    implicitHeight: 30
    readonly property bool overflowing: capsuleRow.implicitWidth > width
    readonly property real anchorOffset: {
      for (var i = 0; i < capsuleRepeater.count; i++) {
        var capsule = capsuleRepeater.itemAt(i)
        if (capsule && capsule.contentItem && capsule.contentItem.anchorOffset >= 0)
          return capsule.x + capsule.horizontalPadding + capsule.contentItem.anchorOffset
      }
      return -1
    }
    ...
    Flickable {
      id: sectionScroller
      anchors.fill: parent
      anchors.rightMargin: section.overflowing ? overflowAffordance.width : 0
      contentWidth: capsuleRow.implicitWidth
        ...
        Repeater {
          ...
          Components.Capsule {
            ...
            contentComponent: Component {
              GroupedModuleList { moduleRows: capsuleGroup.modelData.rows }
            }
          }
        }
      }
    ...
    Text {
      ...
      MouseArea {
        anchors.fill: parent
        cursorShape: Qt.PointingHandCursor
        onClicked: sectionScroller.contentX = sectionScroller.atXEnd
          ? 0 : Math.min(sectionScroller.contentWidth - sectionScroller.width,
                         sectionScroller.contentX + sectionScroller.width)
      }
    }
  }

  // Visual groups retain canonical source regions, so panel numbering,
  // settings updates and drag routing keep their stock semantics.
  component GroupedModuleList: Grid {
    columns: root.vertical ? 1 : Math.max(1, moduleRows.length)
    property var moduleRows: []
    spacing: 0
    readonly property real anchorOffset: {
      for (var i = 0; i < groupedRepeater.count; i++) {
        var slot = groupedRepeater.itemAt(i)
        if (slot && slot.moduleName === root.centerAnchor && slot.width > 0)
    ...
    Repeater {
      id: groupedRepeater
      model: parent.moduleRows

      ModuleSlot {
        required property var modelData
        entry: modelData.entry
        region: modelData.sourceRegion
        sourceIndex: root.sourceIndexFor(modelData.sourceRegion, modelData.entry, modelData.sourceIndex)
      }
    }
  }

  Components.AlwaysVisibleTray { id: trayFactory }
  readonly property Component unboundTrayComponent: root.omarchyPath
    ? trayFactory.create(root.omarchyPath, root) : null
  Components.AlwaysVisibleWorkspaces { id: workspacesFactory }
  readonly property Component unboundWorkspacesComponent: root.omarchyPath
    ? workspacesFactory.create(root.omarchyPath, root) : null
  ...
  component ModuleSlot: Item {
    ...
    readonly property string instanceKey: region + ":" + sourceIndex
    readonly property string moduleName: root.entryId(entry)
    readonly property var moduleSettings: root.entrySettings(entry)
    readonly property string customType: root.customModuleType(entry)
    readonly property var registryMetadata: root.widgetMetadata(moduleName)
    readonly property bool firstParty: root.widgetIsFirstParty(moduleName, registryMetadata)
    property var ownedClickTargets: []
    ...
    readonly property var registryComponent: {
      ...
      if (moduleName === "omarchy.tray" && firstParty) return unboundTrayComponent
      if (moduleName === "omarchy.workspaces" && firstParty) return unboundWorkspacesComponent
    ...
    readonly property real panelIndicatorCenter: {
      var key = root.vertical ? "openPanelIndicatorCenterY" : "openPanelIndicatorCenterX"
      var hint = activeItem && key in activeItem ? Number(activeItem[key]) : NaN
      return isNaN(hint) ? (root.vertical ? slot.height : slot.width) / 2 : hint
    }
    implicitWidth: activeItem && activeItem.visible ? (root.vertical ? root.surfaceSize - 12 : activeItem.implicitWidth) : 0
    ...
    Component.onDestruction: {
      ...
      for (var i = 0; i < ownedClickTargets.length; i++) root.unregisterClickTarget(ownedClickTargets[i])
      for (var j = 0; j < ownedPopouts.length; j++) root.releasePopout(ownedPopouts[j])
      root.unregisterModuleSlot(slot)
    }

    function ownObject(list, object) {
      if (!object || list.indexOf(object) !== -1) return list
      var next = list.slice()
      next.push(object)
      return next
    }

    function disownObject(list, object) {
      return list.filter(function(item) { return item !== object })
    }

    Components.PluginShellFacade {
      id: scopedShellApi
      moduleName: slot.moduleName
      _ownServiceLookup: function() {
        return root.shell && typeof root.shell.serviceFor === "function"
          ? root.shell.serviceFor(slot.moduleName) : null
      }
      _summonOwn: function(payloadJson) {
        return root.shell && typeof root.shell.summon === "function"
          ? root.shell.summon(slot.moduleName, payloadJson) : false
      }
      _hideOwn: function() {
        return root.shell && typeof root.shell.hide === "function"
          ? root.shell.hide(slot.moduleName) : false
      }
      _toggleOwn: function(payloadJson) {
        return root.shell && typeof root.shell.toggle === "function"
          ? root.shell.toggle(slot.moduleName, payloadJson) : false
      }
      _isOwnOpen: function() { return root.isBarWidgetOpen(slot.moduleName) }
      _updateOwnSettings: function(settings) {
        return root.updateScopedEntry(slot.region, slot.sourceIndex, slot.moduleName, settings)
      }
    }

    Components.PluginBarFacade {
      id: scopedBarApi
      pluginId: slot.moduleName
      moduleName: slot.moduleName
      shell: scopedShellApi
      ...
      transparent: root.transparent
      foregroundAnimationEnabled: root.foregroundAnimationEnabled
      centerSectionRevealHeld: root.centerSectionRevealHeld
      _centerHoverRevealSuppressed: root.centerHoverRevealSuppressed
      activePopout: slot.ownedPopouts.indexOf(root.activePopout) !== -1
        ? root.activePopout : (root.activePopout ? foreignPopoutMarker : null)
      clickTargets: slot.ownedClickTargets.slice()
      layoutConfig: root.publicLayoutConfig()
      ...
      _registerClickTarget: function(target) {
        var next = slot.ownObject(slot.ownedClickTargets, target)
        if (next === slot.ownedClickTargets) return
        slot.ownedClickTargets = next
        root.registerClickTarget(target)
      }
      _unregisterClickTarget: function(target) {
        if (slot.ownedClickTargets.indexOf(target) === -1) return
        root.unregisterClickTarget(target)
        slot.ownedClickTargets = slot.disownObject(slot.ownedClickTargets, target)
      }
      _requestPopout: function(owner) {
        slot.ownedPopouts = slot.ownObject(slot.ownedPopouts, owner)
        root.requestPopout(owner)
      }
      _releasePopout: function(owner) {
        if (slot.ownedPopouts.indexOf(owner) === -1) return
        root.releasePopout(owner)
        slot.ownedPopouts = slot.disownObject(slot.ownedPopouts, owner)
      }
      _switchPanelFrom: function(owner, direction) {
        return slot.ownedPopouts.indexOf(owner) !== -1
          ? root.switchPanelFrom(owner, direction) : false
      }
      ...
      _moduleWidgets: function(requestedId) {
        return String(requestedId || "") === slot.moduleName ? root.moduleWidgets(slot.moduleName) : []
      }
      _run: function(command) { root.run(command) }
      _setCenterHoverRevealSuppressed: function(value) { root.centerHoverRevealSuppressed = !!value }
    }
    ...
    Loader {
      ...
      sourceComponent: slot.commandCustom ? customCommandModuleComponent : unknownModuleComponent
      anchors.centerIn: parent
      width: root.vertical && slot.activeItem ? slot.activeItem.implicitWidth : slot.width
      height: slot.height
    ...
    function injectProps() {
      ...
      if (!target) return
      if ("bar" in target) target.bar = firstParty ? root : scopedBarApi
    ...
    Component {
      id: unknownModuleComponent
      Item {
        implicitWidth: unknownLabel.implicitWidth + Style.space(3)
        implicitHeight: Math.max(28, unknownLabel.implicitHeight + Style.space(2))

        Text {
          id: unknownLabel
          anchors.centerIn: parent
          text: "? " + (slot.moduleName || "unresolved")
          textFormat: Text.PlainText
